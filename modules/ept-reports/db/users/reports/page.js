...
module.exports = function(opts) {
  ...
  if (opts && opts.filter && opts.searchStr) { // filter + search
    q = [q, 'WHERE ru.status = $1 AND o.username LIKE $2 ORDER BY', sortField, order, 'LIMIT $3 OFFSET $4'].join(' ');
    params = [opts.filter, opts.searchStr + '%', limit, offset];
  }
  else if (opts && opts.filter && !opts.searchStr) { // filter only
    q = [q, 'WHERE ru.status = $1 ORDER BY', sortField, order, 'LIMIT $2 OFFSET $3'].join(' ');
