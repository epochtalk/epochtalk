...
var insertUserPreferences = function(user, client) {
  var q = 'INSERT INTO users.preferences (user_id, posts_per_page, threads_per_page, collapsed_categories, ignored_boards) VALUES ($1, $2, $3, $4, $5)';
...
var updateUserPreferences = function(user, client) {
  var q = 'UPDATE users.preferences SET posts_per_page = $2, threads_per_page = $3, collapsed_categories = $4, ignored_boards = $5, timezone_offset = $6, patroller_view = $7 WHERE user_id = $1';
  var params = [user.id, user.posts_per_page, user.threads_per_page, user.collapsed_categories, user.ignored_boards, user.timezone_offset, user.patroller_view];
  return client.query(q, params);
};
