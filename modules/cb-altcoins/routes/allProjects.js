...
module.exports = {
  method: 'GET',
  path: '/api/plugins/altcoins/projects/all',
  options: {
    // auth: { strategy: 'jwt' },
    // pre: [ { method: (request) => request.server.methods.auth.altcoins.allProjects(request.server, request.auth) } ]
  },
  handler: function(request) {
    var promise = request.db.altcoins.allProjects()
