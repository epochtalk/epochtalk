...
module.exports = function(conversationId, viewerId, opts) {
  ...
  var columns = 'mid.id, mid.conversation_id, mid.sender_id, mid.receiver_id, mid.copied_ids, mid.body, mid.created_at, mid.viewed, mid.reported, s.username as sender_username, s.deleted as sender_deleted, s.avatar as sender_avatar, r.username as receiver_username, r.deleted as receiver_deleted, r.avatar as receiver_avatar';
  var q = 'SELECT pm.conversation_id, pm.id, pm.sender_id, pm.receiver_id, pm.copied_ids, pm.body, pm.created_at, pm.viewed, CASE WHEN EXISTS (SELECT rm.id FROM administration.reports_messages rm WHERE rm.offender_message_id = pm.id AND rm.reporter_user_id = $2) THEN \'TRUE\'::boolean ELSE \'FALSE\'::boolean END AS reported FROM private_messages pm WHERE conversation_id = $1 AND (pm.sender_id = $2 OR pm.receiver_id = $2) AND pm.created_at <= $3';
